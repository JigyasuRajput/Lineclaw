...
class CastingService {
    ...
    formatRecommendationResponse(talents, category) {
        ...
        let response = `🎯 おすすめタレント（${categoryLabel}）\n`;
        response += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;

        talents.forEach((t, i) => {
            const riskEmoji = t.risk_level === "高" ? "🔴" : t.risk_level === "中" ? "🟡" : "🟢";
            response += `${i + 1}. ${riskEmoji} ${t.name}（${t.name_en || "-"}）\n`;
            response += `   ✅ リスク: ${t.risk_level}\n`;
            response += `   ✅ スキャンダル履歴: ${t.scandal_history}\n`;
            response += `   📞 担当: ${t.expert_contact}（${t.expert_department}）\n\n`;
        });
...
👥 タレント総数: ${stats.totalTalents}名
   🔴 高リスク: ${stats.highRisk}名
   🟡 中リスク: ${stats.mediumRisk}名
   🟢 低リスク: ${stats.lowRisk}名

✅ 起用可能（クリーン）: ${stats.available}名
📄 有効契約数: ${stats.totalContracts}件
🎯 専門家: ${stats.totalExperts}名
...
・「おすすめタレント」→ 推薦リスト`;
    ...
    formatAvailabilityResponse(result) {
        ...
        if (result.available) {
            let response = `✅ ${result.talent.name}さんは起用可能でございます。\n`;
